...
public:
	Figure() { numEdges = 0; figureName = "Фигура"; figureStatus = "Правильная"; };
	virtual ~Figure() = default;

	virtual void printInfoFigure() const
	{
		std::cout << figureStatus << figureStatus << std::endl;
		std::cout << figureName << ". Количество сторон: " << numEdges << std::endl;
	}

	bool checkStatusFigure() {return false};
};
...
public:
	ThrSdFigure() : Figure() {
		numEdges = 3; figureName = "Треугольник", figureStatus = "Правильная";
	...
	bool checkStatusFigure() override {
		if ((A + B + C) == 180) {
...
public:
	RgtAngTriangle() : ThrSdFigure() {
		figureName = "Прямоугольный треугольник"; figureStatus = "Правильная"
